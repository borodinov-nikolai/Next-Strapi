...
const AuthForm: React.FC<Props> = ({registration, setRegistration, setIsModalOpen}) =>{
      ...
      >
        ...
        <div className={styles.text}> Нет аккаунта? <span onClick={()=> {setRegistration(true)}}>Зарегистрируйся!</span></div>
          <div style={{display:'flex', justifyContent: 'end'}}>
            <Button type='primary' style={{marginRight: '-70px', marginTop: '20px'}} onClick={()=>{auth(login, password, router, setIsModalOpen)}} >Авторизоваться</Button>
            </div>
    </Form>
 ) 
...
>
  ...
  >
    ...
    <div className={styles.text} >Есть аккаунт? <span onClick={()=> {setRegistration(false)}} >Авторизуйся!</span></div>
      ...
      <div style={{display:'flex', justifyContent: 'end'}}>
        <Button type='primary' style={{marginRight: '-70px'}} onClick={()=>{register(login, email, password, setSuccess, router, setIsModalOpen)}} >Зарегистрироваться</Button>
